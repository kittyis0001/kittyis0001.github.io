// ═══════════════════════════════════════════════════════════
// STORY EDITOR — Phase 1 (clean, stable)
// Filters · Text · Sticker · Draw · Adjustments
// No crop/zoom/rotation — those Phase 3 changes are removed.
// ═══════════════════════════════════════════════════════════
...
#storyEditorWrap {
  ...
  function bindEditorEvents() {
    ...
    document.addEventListener('keydown', e => {
      if (e.key === 'Enter' && document.activeElement && document.activeElement.id === 'seTextInput') addTextOverlay()
    })
    ...
    document.addEventListener('click', e => {
      if (e.target.id === 'seDrawClear') clearDrawCanvas()
    })
    ...
    document.addEventListener('input', e => {
      if (e.target.id === 'seAdjBrightness') {
        ...
        const v = document.getElementById('seAdjBrightnessVal'); if (v) v.textContent = currentAdj.brightness
        applyPreviewFilter()
      }
      if (e.target.id === 'seAdjContrast') {
        currentAdj.contrast = parseInt(e.target.value)
        const v = document.getElementById('seAdjContrastVal'); if (v) v.textContent = currentAdj.contrast
        applyPreviewFilter()
      }
      if (e.target.id === 'seAdjBlur') {
        currentAdj.blur = parseInt(e.target.value)
        const v = document.getElementById('seAdjBlurVal'); if (v) v.textContent = currentAdj.blur
        applyPreviewFilter()
      }
    })
  ...
  function resetEditor() {
    ...
    if (c) c.value = 100
    if (bl) bl.value = 0
    const bv = document.getElementById('seAdjBrightnessVal'); if (bv) bv.textContent = '100'
    const cv = document.getElementById('seAdjContrastVal');   if (cv) cv.textContent = '100'
    const lv = document.getElementById('seAdjBlurVal');       if (lv) lv.textContent = '0'
    ...
    const noneBg = document.querySelector('#seTextBgColors .se-color-dot[data-bg="none"]')
    if (noneBg) noneBg.classList.add('active')
  ...
  function initDrawCanvas() {
    ...
    const existing = document.getElementById('seDrawCanvas')
  ...
  function addTextOverlay() {
    ...
    const fontEl  = document.getElementById('seTextFont')
    const sizeEl  = document.getElementById('seTextSize')
    const animEl  = document.getElementById('seTextAnim')
    const font    = fontEl ? fontEl.value : 'Arial'
    const size    = sizeEl ? sizeEl.value : '24'
    const anim    = animEl ? animEl.value : 'none'
    ...
    const color   = colDot ? colDot.dataset.col : '#ffffff'
    const bg      = bgDot ? bgDot.dataset.bg : 'none'
        ...
        try {
          ...
          overlayEls.forEach(el => {
            ...
            if (el.style.fontFamily) {
              const fontSizePx = (parseFloat(el.style.fontSize) || 24) * (canvas.width / (previewImg.clientWidth || canvas.width))
              ctx.font = `${fontSizePx}px ${el.style.fontFamily}`
              ...
              const fontSizePx = (parseFloat(el.style.fontSize) || 48) * (canvas.width / (previewImg.clientWidth || canvas.width))
  ...
  window.storyEditorExportEditData = function () {
    ...
    return {
      ...
      adjust: { brightness: currentAdj.brightness, contrast: currentAdj.contrast, blur: currentAdj.blur },
      texts,
      stickers,
